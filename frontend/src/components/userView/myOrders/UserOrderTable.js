...
const UserOrderTable = () => {
    ...
    const getOrders = () => {
        orderService.getUserOrders().then(orders => {
            setOrders(orders);
        }).catch(e => {
            console.log(e);
        })
...
const OrderSearchForm = (props) => {
    ...
    function onSubmit(values) {
        return new Promise(resolve => {
            if(values.title !== ""){
                orderService.searchUserOrdersByTitle(values.title).then(orders => {
                ...
                orderService.searchUserOrdersByDate(from.toISOString(), to.toISOString()).then(orders => {
                    props.setOrders(orders);
                    resolve();
                })
            } else {
